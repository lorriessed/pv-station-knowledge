...
- **2026-06-09 大规模重构** ⚠️:
  - **工商业电站详情组件下线**: 删除 `src/views/station/industryCommerce/` 整个目录（约 10,000+ 行），包括详情页、搜索表单、光伏/储能/零碳三大模块的 20+ 个 Tab 组件、设备曲线、抽屉等。推测该模块正在迁移到新架构（admin-web-next 或 rrsjk-admin-bff）。
  - **年保管理页面下线**: 删除 `src/views/maintainance/guaranteeManage/yearGuarantee/`（984行列表页 + 418行详情弹窗）。
  - **"检出仓库"字段改名为"出库状态"**: 涉及故障工单详情页（招银/中信/协鑫/总部）、OSP运维工单详情、备件监控页。前端展示层改名，后端字段是否同步变更需确认。
  - **分中心优化**: 电费账单页面（电力公司/资方/分中心）下拉和展示逻辑更新。
  - **大屏登录变化**: HTTP 拦截器和登录工具函数修改。
  - **租金停付/恢复**: 新增业务来源字典、租金恢复审核回显优化。
  - **上传进度组件下线**: `uploadProgress.vue` + `uploadProgress.js` 删除。
  - **运维路由精简**: `src/router/maintainance.js` 删除 136 行路由配置。
...
- **2026-06-09 大规模精简** ⚠️:
  - **电站转单页面删除**: `src/views/apv/transferOrder/` 整体删除（193+51行），说明电站转移功能可能已迁移到其他入口（如 Flutter App `feature-station-transfer` 分支）。
  - **录入/受理页面精简**: `enrolment/index.jsx` 从 252 行大幅精简，`accept/index.jsx` 重构。
