...
**Java文件数**: ~44  
**技术栈**: Spring Boot 2.3.3 + Spring Security OAuth2 + Dubbo + Redis + ActiveMQ  
**端口**: 8086, **上下文路径**: `/oauth2`

#### 服务定位
基于 Spring Security OAuth2 的统一认证中心，为所有前端应用（小程序/H5/APP）提供认证和授权服务。JWT Token 签名使用 RSA 私钥/公钥对，Token 存储在 Redis 中。

#### 最近重要变更 (2026-05)

- **SM2 国密算法支持** (2026-05-25, mabin): 添加 SM2 国密算法客户端认证，支持中银租赁场景
- **中银租赁登录** (2026-05-12, mabin): 新增 `ZyTokenRequest`/`ZyTokenResponse` 及中银登录接口
- **中银 Token 合并** (2026-05-26, 于淼): Merge #1 into master from 20260511-zhongyinToken

#### Controller 端点

| Controller | 路径 | 功能 |
|-----------|------|------|
| `AuthController` | `/oauth/login` (POST) | 手机号密码登录 (增强版，支持 VPP Token 自动获取) |
| | `/oauth/sms_login` (POST) | 短信验证码登录 |
| | `/oauth/convertToken` (GET) | 通过 UUID 换取 access_token |
| | `/oauth/hds_idmlogin` (GET) | HDS 统一登录（海尔IDM） |
| | `/oauth/assertLease/login` (POST) | **中银租赁专用登录** (SM2 国密解密密码，client_credentials 授权模式) |
| `OpsAuthController` | `/oauth/ops/login` (POST) | 小象运维 APP 登录 |
| | `/oauth/ops/convertToken` (GET) | 运维 Token 转换 |
| `UserController` | `/principal.do` (GET) | 获取当前用户信息（含角色） |
| `KaptchaController` | `/captcha.do` (GET) | 图形验证码生成 |
| `SmsController` | `/sms/sms_code_login.do` (GET) | 发送短信验证码（加密手机号） |
| | `/sms/send_vertify_code_ln.do` (GET) | 发送短信验证码（明文手机号） |
| `MiniController` | `/wechat/mini/login.do` (POST) | 微信小程序认证 |
| `SmartController` | `/baidu/smart/login.do` (POST) | 百度小程序认证 |
...
- **JWT Token**: RSA 私钥签名(privateKeyJwtAccessTokenConverter)、公钥验证(publicKeyJwtAccessTokenConverter)，Token 中包含 member_id、head_image_url、login_type、name、mobile、lntoken
- **Token 存储**: `RedisAuthorizationCodeServices` — 授权码存储于 Redis，过期时间 10 分钟
- **Session**: Redis 存储 (`EnableRedisHttpSession`，namespace: `rrsjk:session`，有效期 3 天)
- **登录事件**: 登录成功后发送 `VirtualTopic.LOGIN.SUCCESS` ActiveMQ 消息（`LoginSuccessSendMQEventListener`），包含 login_id, member_id, client_id, roles, grant_type
- **登录历史**: `AuthenticationSuccessEventListener` / `AuthenticationFailEventListener` 记录登录成功/失败历史
- **VPP Token 集成** (代码明确证明): 登录时检查用户是否有 `vpp` 角色，自动向 VPP 系统 (`vppTokenUrl`) POST 请求获取 VPP access_token，附在 `Oauth2TokenVo` 返回
- **注销检查**: 登录前通过 `LightLogOffService` 检查用户是否提交注销申请，已注销/审核中超过15天则拒绝登录
- **HDS SSO**: 支持海尔统一身份认证 IDM 登录
- **中银租赁 SM2 登录** (2026-05, mabin, 代码明确证明): `/oauth/assertLease/login` 端点专供中银租赁系统调用。当 `client_id` 以 `zhong_yin_lease` 开头时，从 `oauth2_client` 表的 `additionalInformation` 中取出 SM2 私钥/公钥，用 SM2 私钥解密请求中的加密密码，再与存储的客户端密钥比对。认证通过后使用 `client_credentials` grant_type 签发 OAuth2 Token，返回 `ZyTokenResponse`（含 token/expiresIn/userName/tokenType）。
- **安全管理**: XSS 过滤、RSA 解密手机号、短信防刷(IP/手机号限频、黑名单)、图形验证码

#### 小程序/App 配置 (Oauth2AppConfig)

**来源**: `rrsjk-oauth2-web` → `Oauth2AppConfig.java` (代码明确证明)

已注册的 OAuth2 应用包括：小顺管家(QQ/微信)、日日顺乐农(QQ/微信)、RRSJK(微信扫码/QQ)、乐家诚品小程序、云智妆小程序、健康水站小程序、智慧园区小程序、**日日顺新能源小程序**、**日日顺新能源施工队小程序**、**纳晖储能小程序**、**纳晖工商业预测小程序**、乐农商城H5、百度小程序等。

#### 数据库表

| 表名 | 数据库 | 说明 |
|---|---|---|
| `oauth2_client` | rrsjk_system | OAuth2 客户端配置 |
| `member` | rrsjk_member | 会员信息（Dubbo 调用） |
| `member_role` | rrsjk_member | 会员角色 |
| `login_info` | rrsjk_member | 登录信息 |
| `login_history` | rrsjk_member | 登录历史 |
| `light_log_off` | rrsjk_light | 注销申请记录 |
...
#### 服务定位
统一的异步 Excel 导入导出服务，支持大数据量导出和 OSS 文件存储。

#### ⚠️ 2026-05-27~28 重大重构 (代码明确证明)
**来源**: 解钦, 20+ commits, branch: dev
- **模块拆分**: 拆分为 `rrsjk-async-import-export-api/` + `rrsjk-async-import-export-impl/` 两个子模块
- **MQ 重构**: 删除自研 RocketMQ 封装 (AbstractMqConsumer/RocketMqFactory/UsherMq 等12个文件)，迁移至 usher 框架
- **枚举迁移**: ExportBeanEnum/TaskStatusEnum/TaskTypeEnum 从 impl 移至 api 模块
- **实体迁移**: BaseEntity/AsyncImportExportTask 从 impl 移至 api 模块
- **Dubbo 服务**: AsyncExportTaskDubboService 迁移到 api 模块
- **Controller 删除**: 移除 AsyncImportExportTaskController (不再直接暴露 HTTP 接口)
- **新增 trace 追踪**: logback-spring.xml 配置
- **新增测试**: AsyncExportTaskTest.java
- **领用单导出**: LightUseOrderExportService 调试，双数据源配置优化
- **分页查询**: AsyncImportExportTaskMapper.xml 新增分页查询

#### ⚠️ 2026-06-03 OOM 修复 — 大Excel导出改用临时文件 (代码明确证明)
**来源**: `rrsjk-async-import-export-impl/src/main/java/com/rrsjk/async/service/excel/AsyncExcelUtilService.java` (解钦, commit e8ffef0a, 2026-06-03)
- **问题**: 原实现用 `ByteArrayOutputStream` 将整个Excel写入内存后再上传OSS，大数据量导出时 JVM OOM
- **修复**:
  - 改为 `File.createTempFile("export", ".xlsx")` → `FileOutputStream` 写入磁盘 → `FileInputStream` 上传OSS
  - `SXSSFWorkbook.dispose()` 确保临时数据清理
  - 上传失败时明确返回错误信息 (`StringUtils.isBlank(url)` 校验)
- **模式识别**: 这是典型的内存优化模式 — `ByteArrayOutputStream` → 临时文件 + `SXSSFWorkbook`
- **扫描日期**: 2026-06-08
...
| Controller | 路径 | 功能 |
|-----------|------|------|
| `AsyncImportExportTaskController` | — | 异步导入导出任务管理 (已删除) |
