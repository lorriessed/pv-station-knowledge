...
## 退商负激励 SAP 记账 (代码明确证明, 2026-06-15)
**来源**: `rrsjk-finance-service` + `rrsjk-light-service` (commits: 42f92e09, 77c340bde4, e4222315, 分支 origin/20260610-tuishang, majinhu/马金虎, 2026-06-11~12)
**关联需求**: TAEI-3185 退商流程优化

### 变更内容
1. **新增 A15 业务模式**: `LightSapLedgerQueue.Ywms.A15("A15", "服务商退商负激励")`
2. **SAP 记账逻辑**: `LightSpInspireLedgerModel` 新增 `SP_WA` 类型分支，设置 ywms=A15
3. **触发时机**: 财务确认后生成负激励并记账（异步执行），业务模式传 A15
4. **历史数据处理**: `processHistoricalSpWaInspire` 方法处理历史退商负激励数据
5. **前端适配**: admin-web 新增「待总部确认」状态筛选(CONFIRM_WAIT)、激励管理类型筛选新增「退商负激励」

### 业务流程
退商 → 财务确认 → 异步生成负激励 → SAP 记账(A15) → 总部确认审核

## SAP 记账队列全局暂停结算服务商 (代码明确证明, 2026-06-18)

**来源**: `rrsjk-finance-service` (commit: 893fab29, 龙龙, 2026-06-18)
**关联**: `TempConstants.stopCostSpIdList` (rrsjk-light-base 1.0.0-cbs-SNAPSHOT)

**变更范围**: 在 6 个 SAP 总账记账 Model 中统一新增暂停结算服务商拦截逻辑:
- `LightSapLedgerLineModel` — 主材+辅材服务费记账
- `LightSapLedgerCoinLineModel` — 能源币记账
- `LightSapLedgerSubsidyLineModel` — 电费补贴记账
- `LightSapLedgerServiceBeforeModel` — 服务费前半部分成本记账
- `LightSapLedgerTempLineModel` — 临时任务记账 (1PG0采4110辅料)
- `LightSapLedgerUseServiceModel` — 服务费领用记账

**逻辑**: 在记账前检查 `TempConstants.stopCostSpIdList.contains(station.getSpId())`，命中则将队列状态更新为 `STOP(-1, "临时暂停处理")` 并跳过该条记账。

**暂停服务商ID列表** (8个): 702, 719, 2544, 2557, 2649, 2659, 2669, 2672

**LightSapLedgerQueue.Success 枚举扩展**: 新增 `STOP(-1, "临时暂停处理")`，与原有 `NO(0)` / `YES(1)` 并列。

**⚠️ 注意**: 此前(2026-06-15)已在安装费采购单结算中发现此机制，本次变更将暂停范围从安装费扩展到**所有SAP总账记账队列**，影响面更大。这是硬编码临时方案，服务商变更需重新发版。

## 质保金退款申请审核逻辑修复 (代码明确证明, 2026-06-17)

**来源**: `rrsjk-finance-service` → `PurchaseApplySettleDepositRefundServiceImpl.java` (commits: 65fd1ca3/50d1b633, tn_wangb, 2026-06-17)

**变更1 — 质保金明细状态更新范围扩大**:
- 旧逻辑: 仅对"新增的质保金明细"(addDepositIds)更新为"请款申请中"
- 新逻辑: 对**所有关联的质保金明细**(newDepositRecordIds)统一更新为 `APPLY_SETTLE` 状态
- 删除的明细仍恢复为 `ENABLE_SETTLE`(可结算)

**变更2 — 退款申请删除时增加占用检查**:
- 当查询不到关联质保金明细时，新增错误提示: "未查询到质保金明细，可能被其他退款申请[xxx]占用"
- 通过 `depositRecordDao.getByIds()` 反查明细当前关联的 applyNo

## VPP 电力交易对接 FAP (代码明确证明, 2026-06-15)
**来源**: `rrsjk-light-service` (commit: 53e189a5, 代继宁, 2026-06-12, 分支 origin/20260611-fapForVppElec)
**关联需求**: VPP 电力交易/绿证 FAP 集成

### 变更内容
1. **BizTypeEnum 新增**: `VPP_ORDER("绿证", "TY010029", "收-其他应收款（130）")`
2. **新增接口**:
   - `cancelFapOrderByOrderNoAndBizType(orderNo, bizType)` — 按业务单号+业务模式作废 FAP 订单
   - `queryVppFapRecordByOrderNoAndBizType(orderNo, bizType)` — 按业务单号+业务模式查询 VPP FAP 记录
3. **设计意图**: 随着 FAP 业务类型增多，防止不同业务模块单号冲突，统一用"业务单号+业务模式"作为唯一标识

### ⚠️ 枚举重复定义风险 (代码明确证明)
commit 53e189a5 中 `VPP_ORDER` 出现两行完全相同的定义（merge 残留）：
```java
VPP_ORDER("绿证", "TY010029", "收-其他应收款（130）"),
VPP_ORDER("绿证", "TY010029", "收-其他应收款（130）"),
```
**影响**: Java 枚举不允许重复常量名，此代码**无法编译通过**。需确认是否已在后续提交修复。

### FAP 回调路由架构更新
LightFapRecordServiceImpl 作为 FAP 回调中心，当前 BizTypeEnum 已覆盖:
- 运维保证金 / 备件保证金 / 零碳保证金
- 零碳订单 / 电费收益
- 经营性租赁 / 基金回款
- 商城订单
