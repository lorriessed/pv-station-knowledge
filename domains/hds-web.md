...
## HDS 运维 Controller 大规模清理 (代码明确证明, 2026-06-29)

**来源**: `rrsjk-hds-web` commit `31446df6` (sunzn, 2026-06-29, Merge `szn_station_elec_report_20260611`)

### 删除的 Controller（12个）
| Controller | 功能 | 备注 |
|---|---|---|
| `LightOperationStationInsuranceController` | 电站保险管理 | 271行删除 |
| `LightOperationStationAnnualGuaranteeController` | 电站年保管理 | 157行删除 |
| `LightOperationStationTagDefinitionController` | 电站标签定义 | 157行删除 |
| `LightOperationZeroCarbonEStationController` | 零碳E站管理 | 79行删除 |
| `LightOperationZeroCarbonStationController` | 零碳电站管理 | 154行删除 |
| `LightSpOpsNegativeExcitationController` | 服务商负激励 | 86行删除 |
| `LightSpOpsNegativeIacController` | 服务商负激励IAC | 99行删除 |
| `LightSpOpsPositiveExcitationController` | 服务商正激励（精简） | 从107行减至~20行 |
| `LightSpOpsPositiveIacController` | 服务商正激励IAC | 108行删除 |
| `EvaluateController` | 评价管理 | 93行删除 |
| `SysEvaluateController` | 系统评价 | 96行删除 |
| `RepairsAndRrsController` | 维修与RRS关联 | 33行删除 |

### 删除的 DTO（5个）
- `StationAnnualGuaranteeExportDto` (200行)
- `StationInsuranceImportDto` (29行)
- `StationInsuranceRecordExportDto` (69行)
- `StationTagDefinitionExportDto` (75行)
- `OperationMaintenanceStationExcel` (66行)

### 新增/变更
- **新增**: `CreateOperationMaintenanceStationController` — 运维电站创建（含月度账单导入、A49冲销暂估、A51特殊标记处理等）
- **数据源切换**: `EnergyAuditStatusReportController` 使用新分中心数据源 (`fix(hds): use new sub-center source`)
- **清理**: `LightStationController.showRecord.do` 移除审核记录中姓名脱敏的正则逻辑（注释掉，约40行）
- **配置**: 移除远程调试参数 (`-agentlib:jdwp=...address=0.0.0.0:8091`)
- **service.xml**: 80行 Dubbo reference 调整

### 业务含义
HDS 平台正在进行功能迁移/下线 — 保险、年保、标签定义、零碳E站、服务商激励等功能可能已迁移到 `rrsjk-admin-bff` 或其他新服务。这是 admin-web-next 迁移计划的一部分。
- 库存监控页面取消重复的查询条件
